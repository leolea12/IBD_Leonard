...
selection_taxon <- dt_Chorologie %>% select(full_name) %>% unique() %>% arrange(full_name)
...
ui <- fluidPage(theme = shinytheme("united"),
                navbarPage("Barre de Naviguation",
                tabPanel("Temporalité",
                sidebarLayout(             
  sidebarPanel(selectInput("categorical_variable",
                           label = "Choix du code taxon :",
                           choices = selection_taxon,
                           selectize = FALSE),
               code("Taxons appariés: "), code(textOutput("name_list")),
               plotOutput("Plot1", height = "300"), plotOutput("Plot2", height = "300")),
  mainPanel(
      tabPanel("Carte", fluidRow(leafletOutput("mapFiltered",width = "1200", height = "900")))))),
  tabPanel("Données",dataTableOutput("Donnees", width = "1800"), downloadButton("downloadData", "Download")),
  
  navbarMenu("Compléments",
...
server <- function(input, output){
  ...
  mapFiltered <- reactive({
    titre <- dt_Chorologie %>% 
      ...
      pull(name_valid) %>% unique()
    
    map <- map_base %>% addCircles(data = dt_Chorologie %>% filter(full_name %in% input$categorical_variable) %>% 
  ...
  output$mapFiltered <- renderLeaflet({mapFiltered()})
  
  output$name_list <- renderText({as.character(dt_Chorologie %>% 
                                                 filter(full_name %in% input$categorical_variable) %>% 
                                                 select(taxons_apparies) %>% 
                                                 unique() %>% 
                                                 mutate(taxons_apparies = if_else(is.na(taxons_apparies) == T, "Aucun", taxons_apparies)))})
  
  output$Plot1 <- renderPlot({dt_Chorologie %>%
      filter(full_name %in% input$categorical_variable) %>%
      mutate(annee = as.factor(annee)) %>% 
      group_by(annee) %>% 
      ...
      geom_line(size = 0.5)+
      geom_point(size = 2)+
      labs(title = "Importance du taxon dans les échantillons",
           x = "Année", y = "Abondance relative (1/N)")+
      InraeThemes::theme_inrae()})
  
  output$Plot2 <- renderPlot({dt_Chorologie %>%
      filter(full_name %in% input$categorical_variable) %>% 
      mutate(annee = as.factor(annee)) %>% 
      group_by(annee) %>% 
      ...
      geom_line(size = 0.5)+
      geom_point(size = 2)+
      labs(title = "Nombre de stations de recensement",
           x = "Année",y = "Occurence")+
      InraeThemes::theme_inrae()})
  
  output$Donnees <- renderDataTable(datatable(dt_Chorologie %>% 
                                                filter(full_name %in% input$categorical_variable) %>% 
                                                select("ANNEE" = annee, CODE = code_taxon, NOM = name_valid, STATION = CODE_STATION, 
                                                       COMMUNE = Commune, ABONDANCE = RESULTAT, ABONDANCE_RELATIVE = ab_rel, TOTAL = Tot_indiv), 
                                              extensions = 'Buttons',
                                              options = list(pageLength = 20, dom = 'lftipB',
                                                             buttons = c('pdf'),
                                                             scroller = TRUE)))
  
  output$Plot_2007 <- renderPlot({plot(dt_Chorologie, 2007)}, height = 800, width = 1700)
  output$Plot_2008 <- renderPlot({plot(dt_Chorologie, 2008)}, height = 800, width = 1700)
  output$Plot_2009 <- renderPlot({plot(dt_Chorologie, 2009)}, height = 800, width = 1700)
  output$Plot_2010 <- renderPlot({plot(dt_Chorologie, 2010)}, height = 800, width = 1700)
  output$Plot_2011 <- renderPlot({plot(dt_Chorologie, 2011)}, height = 800, width = 1700)
  output$Plot_2012 <- renderPlot({plot(dt_Chorologie, 2012)}, height = 800, width = 1700)
  output$Plot_2013 <- renderPlot({plot(dt_Chorologie, 2013)}, height = 800, width = 1700)
  output$Plot_2014 <- renderPlot({plot(dt_Chorologie, 2014)}, height = 800, width = 1700)
  output$Plot_2015 <- renderPlot({plot(dt_Chorologie, 2015)}, height = 800, width = 1700)
  output$Plot_2016 <- renderPlot({plot(dt_Chorologie, 2016)}, height = 800, width = 1700)
  output$Plot_2017 <- renderPlot({plot(dt_Chorologie, 2017)}, height = 800, width = 1700)
  output$Plot_2018 <- renderPlot({plot(dt_Chorologie, 2018)}, height = 800, width = 1700)
  output$Plot_2019 <- renderPlot({plot(dt_Chorologie, 2019)}, height = 800, width = 1700)
  output$Plot_2020 <- renderPlot({plot(dt_Chorologie, 2020)}, height = 800, width = 1700)
  output$Plot_2021 <- renderPlot({plot(dt_Chorologie, 2021)}, height = 800, width = 1700)
